...
abstract class BasicDatabaseHelper
{
    ...
    public function __construct(bool $manualCall = false, string $host = null, string $name = null, string $username = null, string $password = null)
    {
        if ($manualCall)
        {
            $this->pdoConn = new PDO("mysql:host=$host;dbname=$name", $username, $password);
            $this->database = $name;
    ...
    public function Select(array $_where): bool | stdClass
    {
        $whereSQL = array();
        $whereSQLCounter = 0;
        foreach ($_where as $key => $value)
